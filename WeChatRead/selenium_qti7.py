...
class SeleniumBaseClass(object):
    ...
    def RollingRoller(self, handle):
        '''
        @Time    :   2023/12/26 18:36:39
        @功能    :   滚动条滚到某个元素
        '''
        # 创建一个ActionChains实例
...
class selenium_qti(SeleniumBaseClass):
    ...
    def __init__(self,
        ...
        self.virtual_chrome = virtual_chrome #默认不启用虚拟浏览器
    ...
    def NewGetOwnerEmail(self,name):
        # self.browser.switch_to.frame(self.browser.find_element_by_tag_name("iframe"))
        self.browser.switch_to.parent_frame()
        self.WaitLoadingInfo('//div/div/div[1]/div/div[2]/span/a')
        self.browser.implicitly_wait(5)
        ...
        while count < 3:
            ...
            for i in range(1,len(table_rows)+1):
                # handles = self.browser.find_elements(By.XPATH, f'//div/div/div/div/ul[2]/li[{i}]/a/span[2]')
            ...
            # button_handles = self.browser.find_elements(By.XPATH, f'//div/div/div/div/ul[2]/li[{index}]/div[2]/button/lightning-primitive-icon')
            button_handles = self.GetHandles(xpath=f'//div/div/div/div/ul[2]/li[{index}]/div[2]/button/lightning-primitive-icon')
            try:
                # print(len(button_handles))
