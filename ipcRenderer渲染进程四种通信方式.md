好的，让我们更详细地解释 `ipcRenderer.on`、`ipcRenderer.invoke`、`ipcRenderer.send` 和 `ipcRenderer.sendSync` 的使用场景、优缺点，并提供详细的代码实例。
...
### 总结
- **`ipcRenderer.on`**：适用于持续监听事件的场景。
- **`ipcRenderer.invoke`**：适用于异步请求-响应模式。
- **`ipcRenderer.send`**：适用于发送异步通知或触发事件。
- **`ipcRenderer.sendSync`**：适用于需要立即获取响应的同步操作。
